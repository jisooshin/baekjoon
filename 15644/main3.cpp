...
Info findWays()
{
	...
	while(!q.empty())
	{
		...
		for (const auto direction: directionLoop)
		{
			...
			Coord targetRedCoord  = { target.red.I + td(dirChar).I, target.red.J + td(dirChar).J};
			Coord targetBlueCoord = { target.blue.I + td(dirChar).I, target.blue.J + td(dirChar).J};
			Info candidateInfo = target;

			// Red ball  Blue ball 둘중 하나라도 움직이면 Queue에는 넣어주는 걸로.. 
			if (maze[targetRedCoord.I][targetRedCoord.J] == '#' && maze[targetBlueCoord.I][targetBlueCoord.J] == '#') continue;

			if (visited[targetRedCoord.I][targetRedCoord.J] == false)
			{
				...
				// if (target.red == candidateInfo.red) { continue; } 
